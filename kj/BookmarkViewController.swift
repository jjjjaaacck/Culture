...
class BookmarkViewController: UIViewController{
    ...
    @IBAction func segmentedControlClick(_ sender: UISegmentedControl) {
        (sender.selectedSegmentIndex == 0) ? (tempTableViewModel = tableViewModel) : (tempTableViewModel = tableViewModelForMonth)
...
//    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showBookmarkDetail" {
            ...
            detailController.category = ((sender as! NSArray).object(at: 0)) as! Int
            detailController.id = ((sender as! NSArray).object(at: 1)) as! String
        }
    }

...
class MyMapViewController: UIViewController,CLLocationManagerDelegate ,GMSMapViewDelegate{
    ...
    override func viewDidLoad() {
        ...
        mapViewYo.addObserver(self, forKeyPath: "myLocation", options: NSKeyValueObservingOptions.new, context: nil)
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if !didFindMyLocation {
            if let myLocation : CLLocation = change?[NSKeyValueChangeKey.newKey] as? CLLocation
            {
                ...
                self.mapViewYo.camera = GMSCameraPosition.camera(withTarget: self.currentLocation, zoom: 15.0)
                self.mapViewYo.settings.myLocationButton = true
